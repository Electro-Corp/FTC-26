...
| `subsystems/Shooter.java` | modified | Added `retargetVelocity()` which re-applies `SPINNER_SPEED_NEAR` to the flywheels when the shooter is in a forward-spinning state (`SPIN_UP_HOLD`, `WAITING_FOR_SPIN_UP`, `SHOOTING`). No-op in `STOPPED` / `REVERSE` so it doesn't fight other commands. Added `firingSessionActive` flag and rewrote the dam policy in `update()`: dam-up by default (idle, intaking, pre-shot spin-up holds), dam-down for the duration of an active firing session including the brief inter-shot SPIN_UP_HOLD windows. Cleared by `stopShoot()`. Fixes auto-mode dam staying down the whole match (balls dribbled into the flywheel during intake) without bouncing the dam up-down between every ball in a 3-shot sequence. |
